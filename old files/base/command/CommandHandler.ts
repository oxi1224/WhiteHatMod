...
import type { ClassConstructor, CommandHandlerOptions, ParsedArgs } from "../lib/types.js";
...
export class BaseCommandHandler extends EventEmitter {
  ...
  constructor(
    client: CustomClient,
    { commandExportFile, prefix = "!", flagRegex = /--.+/, aliasReplacement }: CommandHandlerOptions
  ) {
    super();
    this.client = client;
  ...
  private async loadAll() {
    const imports: { [key: string]: ClassConstructor<BaseCommand> } = await import(
  ...
  public async handleSlash(interaction: Interaction): Promise<unknown> {
    ...
    if (!(interaction.member as GuildMember).permissions.has(command.userPermissions))

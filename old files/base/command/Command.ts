...
import { CommandInteraction, PermissionResolvable, type Message } from "discord.js";
...
export abstract class BaseCommand {
  ...
  public async parseArgs(
    ...
    for (let i = 0; i < argumentArray.length; i++) {
      ...
      if (contentArray[arg.index]) {
        switch (arg.type as CommandArgumentType) {
          case "user":
            ...
            value = await message.client.users
              .fetch(contentArray[arg.index].replace(/[\\<>@#&!]/g, ""))
              .catch(() => null);
            break;
          case "channel":
            ...
            value = await message.guild?.channels
              .fetch(contentArray[arg.index].replace(/[\\<>@#&!]/g, ""))
              .catch(() => null);
            break;
          case "role":
            value =
              ...
              message.guild?.roles.cache.find(
                (role) => role.name === contentArray[arg.index].replace(/[\\<>@#&!]/g, "")
              ) ??
              null;
            break;
          case "string":
            value = contentArray
              .slice(arg.index, arg.index + arg.length)
              .filter((str) => !this.commandHandler?.flagRegex.test(str))
              .join(" ");
            break;
          case "integer":
          ...
          case "duration":
            ...
            const matchingDuration = regex.duration.test(stringOne)
              ? stringOne
              : regex.duration.test(stringTwo)
                ? stringTwo
                : null;
      ...
      let newIndex = i;
      newIndex++;
      if (!value && !arg.required && argumentArray[newIndex] !== undefined)
        argumentArray[newIndex].index = i;
  ...
  public parseSlash(interaction: CommandInteraction, argumentArray: Argument[]): ParsedArgs | null {
    ...
    argumentArray.forEach((arg) => {
      ...
      if (arg.type === "duration")
        return (args[arg.id] = parseDuration(
